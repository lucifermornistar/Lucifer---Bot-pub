...
try {
  mongoose.connect(process.env.CREDENCIAL_DB, { useNewUrlParser: true, useUnifiedTopology: true });
} catch(err) {
  console.error(err)
}
...
try{
  ...
  bot.login();//faz login na API do discord com a credencial do bot
}catch(e){
  console.log("ERRO AO TENTAR LOGAR NA API DO DISCORD: ", e); ///mandar msg pra outro bot avisando disso
  //console.log(e);
}
//mongodb+srv://LordLuchzin:<password>@cluster0-duppc.mongodb.net/<dbname>?retryWrites=true&w=majority